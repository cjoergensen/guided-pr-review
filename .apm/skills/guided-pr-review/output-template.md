...
- A posted comment always carries a "Reviewed by: [name]" line — this is a record of whose decisions these were, not the process's own verdict. An "Additional notes" section appears only when the reviewer supplied text for it, and that text is theirs verbatim, not a rephrasing of the discussion above.
