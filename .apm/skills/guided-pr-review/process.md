...
7. **If the runtime is VS Code** (or a fork), ask once, up front, whether to build a live [CodeTour](https://marketplace.visualstudio.com/items?itemName=vsls-contrib.codetour) `.tour` file as the walkthrough proceeds, so the reviewer can follow along in the editor (see `principles.md`, "Offer a .tour export where it fits, never assume it"). Never ask on a runtime that isn't VS Code. A yes here carries through Phases 3 and 4 below; a no (or a runtime where this wasn't asked) still leaves the one-shot fallback in Phase 6 available.
...
If the reviewer opted into a live tour in Phase 1, nothing needs initializing here — unlike the whole-PR tour Phase 6 eventually builds, live mode writes one small, single-step `.tour` file per component, lazily, as Phase 4 reaches each one.
...
- **If a live tour was opted into in Phase 1, write this component's own single-step `.tour` file now** — before raising the discussion point below, not after resolving it. Name it `.tours/<pr-slug>/<i>-<component-slug>.tour`. Content is a one-line description for a clean, low-risk component, or a fuller one built from Role/Risk/Look for (and the discussion question itself, once it's written) for anything else — same collapse logic as the chat output. The point is giving the reviewer a way to look at the actual code before they weigh in, not logging what happened afterward (that's `review-notes.md`, the summary table, and the Phase 6 whole-PR tour's job). Link to this component's own file right here, after "Look for" and before "Discussion" — using the runtime's file-open URI (e.g. `vscode://file/{path-to-this-step's-tour-file}`), which opens it in VS Code for CodeTour to prompt starting it. Because the file has exactly one step, opening it is unambiguous — no step-jump URI needed, verified or otherwise.
...
**If live per-component tours were built** (Phase 1/4), also assemble the whole-PR tour now, at `.tours/<pr-slug>.tour` — this needs no new ask, since it's the same up-front consent, not a fresh request. It covers the same components in the same order as the individual step files, but unlike those, it's built after everything is resolved, so each step's description can and should include how the discussion point was settled, not just what it asked. Say so and link to it; don't ask again. The individual per-component files stay too — they're still the exact links used during the walkthrough, and this consolidated file doesn't replace them, it's for browsing the whole review in one sequential pass afterward.

**Otherwise, if the runtime is VS Code** (or a fork) and no live tour was started, separately ask whether to export the whole-PR `.tour` file now, built in one shot from `review-notes.md` (see `principles.md`, "Offer a .tour export where it fits, never assume it") — this is a distinct yes/no from the PR-posting choice above; saying yes to one doesn't imply the other, and don't ask at all on a runtime that isn't VS Code. If the reviewer says yes, write one step per component from the Phase 3 list, in the same order, at `.tours/<pr-slug>.tour`, including each one's resolution since it's already known. Write it only after this explicit go-ahead — never silently.
